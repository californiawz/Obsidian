...
- `package` 名统一小写，用点分割
- 业务内部的各个模块 package，必须有二级 package 名，比如 `txad.adx`、`txad.feeds` 等
...
- 避免使用 `uint32` 和 `uint64`（Java 等语言不支持）
- 浮点数优先用 `double`
...
- 请勿删除已上线的字段，应将其标注为 `reserved`
- 新增字段不得使用已被 `reserved` 的字段编号
